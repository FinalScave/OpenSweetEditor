...
namespace NS_SWEETEDITOR {
  ...
  PointF EditorInteraction::resolveScaleFocus(const GestureEvent& event) const {
    if (event.points.size() >= 2) {
      return {(event.points[0].x + event.points[1].x) * 0.5f, (event.points[0].y + event.points[1].y) * 0.5f};
    }
  ...
  EditorInteraction::PointerInteractionOwner EditorInteraction::hitTestHandle(const PointF& screen_point) const {
  ...
  void EditorInteraction::dragHandleTo(PointerInteractionOwner target, const PointF& screen_point) {
    if (!m_context_.caret->hasSelection()
        || (target != PointerInteractionOwner::HANDLE_START && target != PointerInteractionOwner::HANDLE_END)) {
      return;
    }
    ...
    if (sel_end < sel_start) {
      ...
      m_pointer_interaction_.owner = target == PointerInteractionOwner::HANDLE_START
  ...
  void EditorInteraction::dragSelectTo(const PointF& screen_point, bool is_mouse) {
    PointF adjusted_point = screen_point;
    if (!is_mouse) {
      const float hit_bottom =
          std::max(m_context_.settings->handle.start_hit_area.bottom, m_context_.settings->handle.end_hit_area.bottom);
      adjusted_point.y -= hit_bottom;
    }

    const CaretHit hit = m_context_.text_layout->hitTestTextBoundary(adjusted_point);
    const TextPosition anchor = m_context_.caret->hasSelection() ? m_context_.caret->anchor : m_context_.caret->active;
  ...
  void EditorInteraction::updateEdgeScrollState(const PointF& screen_point, bool selection_drag, bool is_mouse) {
  ...
  InteractionResult EditorInteraction::handleGestureEvent(const GestureEvent& event) {
    ...
    if (event.type == EventType::DIRECT_GESTURE_END) {
      ...
      m_interaction_lifecycle_.direct_viewport_gesture_depth--;
      if (m_interaction_lifecycle_.direct_viewport_gesture_depth == 0 && m_context_.view_state != nullptr) {
        ...
        if (m_context_.text_layout != nullptr && m_context_.viewport != nullptr
    ...
    if (primary_down) {
      interaction_result.handled = m_pointer_interaction_.owner != PointerInteractionOwner::NONE;
    ...
    case GestureType::SCALE: {
      ...
      m_context_.view_state->scale =
          std::max(m_context_.settings->min_scale,
                   std::min(m_context_.settings->max_scale, m_context_.view_state->scale * result.scale));
      break;
    }
    ...
    const bool scale_gesture_end = m_scale_gesture_active_
                                   && (event.type == EventType::TOUCH_POINTER_UP || event.type == EventType::TOUCH_UP
                                       || event.type == EventType::TOUCH_CANCEL);

    if (scale_gesture_end || (!m_scale_gesture_active_ && isPrimaryPointerEnd(event.type))) {
      m_context_.view_state->scroll_x = std::round(m_context_.view_state->scroll_x);
      m_context_.view_state->scroll_y = std::round(m_context_.view_state->scroll_y);
    }
    ...
    interaction_result.handled = interaction_result.handled || result.type != GestureType::UNDEFINED;
  ...
  InteractionAnimationState EditorInteraction::resolveAnimationState() {
    ...
    if (fading_in || fading_out) {
      ...
      state.needs_redraw = true;
                 ...
                 & (static_cast<uint32_t>(AnimationFlag::EDGE_SCROLL) | static_cast<uint32_t>(AnimationFlag::FLING)))) {
      state.next_tick_delay_ms = static_cast<uint32_t>(std::max<int64_t>(1, delay_ms - elapsed_since_last));
    }
    return state;
  }
  ...
  void EditorInteraction::onViewportChanged() {
    ...
    const bool handle_active = m_pointer_interaction_.owner == PointerInteractionOwner::HANDLE_START
  ...
  void EditorInteraction::onScrollbarConfigChanged() {
    const bool scrollbar_active = m_pointer_interaction_.owner == PointerInteractionOwner::SCROLLBAR_VERTICAL
                                  || m_pointer_interaction_.owner == PointerInteractionOwner::SCROLLBAR_HORIZONTAL;
    if (scrollbar_active) {
      resetPointerState();
  ...
  bool EditorInteraction::canShowTransientScrollbar() const {
    if (m_context_.settings == nullptr || m_context_.settings->scrollbar.mode != ScrollbarMode::TRANSIENT
        || m_context_.text_layout == nullptr || m_context_.viewport == nullptr
        || !isValidViewportSize(*m_context_.viewport)) {
      return false;
    }
  ...
  void EditorInteraction::computeScrollbarModels(ScrollbarModel& vertical, ScrollbarModel& horizontal) const {
    ...
    const int64_t now_ms = monotonicNowMs();
    const auto axisAlpha = [&](bool logical_visible, PointerInteractionOwner owner) -> float {
    ...
    const float vertical_alpha = axisAlpha(logical_vertical, PointerInteractionOwner::SCROLLBAR_VERTICAL);
    const float horizontal_alpha = axisAlpha(logical_horizontal, PointerInteractionOwner::SCROLLBAR_HORIZONTAL);
    ...
    if (show_vertical && vertical_track_height > 0.0f) {
      ...
      vertical.thumb_active = m_pointer_interaction_.owner == PointerInteractionOwner::SCROLLBAR_VERTICAL;
    ...
    if (show_horizontal && horizontal_track_width > 0.0f && horizontal_track_y >= 0.0f) {
      ...
      horizontal.thumb_active = m_pointer_interaction_.owner == PointerInteractionOwner::SCROLLBAR_HORIZONTAL;
  ...
  bool EditorInteraction::handleSelectionHandleGesture(const GestureEvent& event, InteractionResult& result) {
    const bool active = m_pointer_interaction_.owner == PointerInteractionOwner::HANDLE_START
                        || m_pointer_interaction_.owner == PointerInteractionOwner::HANDLE_END;
    if (!active) {
      return false;
    }
    ...
    if (event.type == EventType::TOUCH_POINTER_DOWN || isPrimaryPointerEnd(event.type)) {
  ...
  bool EditorInteraction::handleScrollbarGesture(const GestureEvent& event, InteractionResult& result) {
    const bool active = m_pointer_interaction_.owner == PointerInteractionOwner::SCROLLBAR_VERTICAL
                        || m_pointer_interaction_.owner == PointerInteractionOwner::SCROLLBAR_HORIZONTAL;
    ...
    if (active && (event.type == EventType::TOUCH_POINTER_DOWN || isPrimaryPointerEnd(event.type))) {
      const bool primary_end = isPrimaryPointerEnd(event.type);
      if (m_context_.view_state != nullptr) {
        m_context_.view_state->scroll_x = std::round(m_context_.view_state->scroll_x);
        m_context_.view_state->scroll_y = std::round(m_context_.view_state->scroll_y);
    ...
    case EventType::MOUSE_DOWN: {
      ...
      if (vertical.visible && m_context_.settings->scrollbar.thumb_draggable
          && pointInRect(point, vertical.thumb, thumb_hit_padding)) {
        m_pointer_interaction_.owner = PointerInteractionOwner::SCROLLBAR_VERTICAL;
        m_pointer_interaction_.start_point = point;
        m_pointer_interaction_.start_scroll_y = m_context_.view_state->scroll_y;
        m_pointer_interaction_.travel_y = std::max(0.0f, vertical.track.height - vertical.thumb.height);
      ...
      if (horizontal.visible && m_context_.settings->scrollbar.thumb_draggable
          && pointInRect(point, horizontal.thumb, thumb_hit_padding)) {
        m_pointer_interaction_.owner = PointerInteractionOwner::SCROLLBAR_HORIZONTAL;
        m_pointer_interaction_.start_point = point;
        m_pointer_interaction_.start_scroll_x = m_context_.view_state->scroll_x;
        m_pointer_interaction_.travel_x = std::max(0.0f, horizontal.track.width - horizontal.thumb.width);
      ...
      if (vertical.visible && m_context_.settings->scrollbar.track_tap_mode == ScrollbarTrackTapMode::JUMP
          && pointInRect(point, vertical.track)) {
      ...
      if (horizontal.visible && m_context_.settings->scrollbar.track_tap_mode == ScrollbarTrackTapMode::JUMP
          && pointInRect(point, horizontal.track)) {
        if (horizontal.track.width > 0.0f && bounds.max_scroll_x > 0.0f) {
          ...
          const float ratio =
              travel <= 0.0f
                  ? 0.0f
                  : std::clamp((point.x - horizontal.track.origin.x - horizontal.thumb.width * 0.5f) / travel, 0.0f,
                               1.0f);
    ...
    case EventType::MOUSE_MOVE: {
      ...
      if (m_pointer_interaction_.owner == PointerInteractionOwner::SCROLLBAR_VERTICAL) {
        ...
        if (m_pointer_interaction_.travel_y > 0.0f && m_pointer_interaction_.max_scroll_y > 0.0f) {
          const float delta = point.y - m_pointer_interaction_.start_point.y;
          target_y += delta * m_pointer_interaction_.max_scroll_y / m_pointer_interaction_.travel_y;
        }
      ...
      if (m_pointer_interaction_.owner == PointerInteractionOwner::SCROLLBAR_HORIZONTAL) {
        ...
        if (m_pointer_interaction_.travel_x > 0.0f && m_pointer_interaction_.max_scroll_x > 0.0f) {
          const float delta = point.x - m_pointer_interaction_.start_point.x;
          target_x += delta * m_pointer_interaction_.max_scroll_x / m_pointer_interaction_.travel_x;
        }
