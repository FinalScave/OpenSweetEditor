...
`text_changes` 是本次调用实际写入 `Document` 的精确变化；列表非空即表示文本发生变化。IME composition 的建立、更新和取消只要改写了当前文本，也会立即出现在该列表中。
...
`interaction_flags` 是 `InteractionFlag` 位集合：`PRIMARY_POINTER=1`、`SELECTION_DRAG=2`、`VIEWPORT_GESTURE=4`。它描述仍在进行的输入生命周期，与动画调度相互独立。
...
`StartLinkedEditingPayload` 直接包含 tab stop 分组：
...
`begin_session` 与 `get_state` 返回 `ImeState`，`get_context` 返回 `ImeTextContext`，session 结束和两个 batch mutation API 返回 `EditorActionResult`。每个 session 的 `ImeMutationModel` 固定不变：命令式接入发送 `ImeCommandBatch`，Flutter delta 接入发送 `ImeTextUpdateBatch`。
