...
| OHOS | 预留目录 | 🚧 未接入 | `platform/OHOS` 当前仅占位 |

## 当前平台层约定

- 控件层公开 API 优先语义化枚举（`WrapMode`、`FoldArrowMode`、`SpanLayer` 等）。
- 桥接层保持原生数值协议（`int`/`byte`）对接 JNI/FFM/PInvoke/C bridge。
- `FontStyle` 等位标志保持常量风格，不改成互斥枚举。

## 建议阅读顺序

1. 先看对应平台控件层 API（对业务最直接）。
2. 再看平台桥接层（JNI/FFM/PInvoke/Swift bridge）。
3. 需要确认 ABI、二进制 payload 布局或枚举值时回到 [C++ 核心 / C API](./api-editor-core.md)。
