...
> This document defines the conventions and constraints that every SweetEditor integration implementation must follow.
> The goal is to keep behavior consistent across implementations while allowing target-specific rendering and input handling.
>
> This document is the normative target contract for integration implementations. It does not claim that every current implementation already satisfies every requirement. If an API or wire-layout detail differs from the source, the source is authoritative and the document must be corrected.
>
> Constraint levels:
> - **MUST** — all implementations must comply; violation is a bug.
> - **SHOULD** — recommended; deviation requires documented justification.
> - **MAY** — optional; each implementation decides based on its own needs.

---

## 1. Module Structure

Every integration implementation MUST include the following two logical layers and ensure all types within each category are implemented. File organization (directory structure, file granularity) MAY vary by language convention (e.g. Java uses package directories, C# uses one file per namespace, Swift uses Sources layering), but the logical categories MUST be identifiable and the types MUST be fully covered.
...
Core protocol types and `CoreProtocol` codecs MUST be generated from the C++ `SE_PROTOCOL_*` annotations through `tools/se_protocol_gen`. Integration code MAY add generated-code augmentation or thin host-language helpers, but MUST NOT maintain independent `ProtocolEncoder` / `ProtocolDecoder` implementations or an implementation-defined binary schema. Augmentation MUST NOT change encoded field order, scalar width, nullability, list/map layout, enum values, or payload names.

### 1.2 Widget Layer (UI Controls / Rendering / Interaction)

The Widget layer handles host-native rendering, user interaction, and extension systems.
...
| **ContextMenu** *(SHOULD on desktop / mouse / right-click targets; MAY on touch-only targets)* | `ContextMenuRequest`, `ContextMenuSection`, `ContextMenuItem`, `ContextMenuItemProvider`, `ContextMenuTriggerKind`, a host-visible custom-item click callback mechanism; MAY: `ContextMenuPopup` | Integration-side context menu / action menu |
| **Perf** *(MAY)* | Debug performance overlay controls, such as `setPerfOverlayEnabled` / `isPerfOverlayEnabled` or integration-equivalent APIs | Debug performance overlay |

> `TextChangedEvent` MUST expose `changes: List<TextChange>`, `kind: TextChangeKind`, and `source: EditorActionSource`. Implementations MUST NOT define a separate text-change action enum that mirrors `TextChangeKind`.

### 1.3 Internal Implementation Freedom (SHOULD)

Internal renderers, popup controllers, overlay controllers, and similar objects are implementation details, not public API contract. Implementations MAY organize these objects according to their UI framework conventions; as long as sections 1.1, 1.2, and the later public contracts are satisfied, not using a specific internal controller / renderer name is not a violation.
...
The widget entry class MUST use `SweetEditor` as prefix and MAY append the host UI toolkit's conventional component suffix:

| Suffix | Full Name | When to Use | Examples |
|---|---|---|---|
| (none) | `SweetEditor` | UI stack does not mandate a component suffix | Android View, Swing, OHOS ArkUI |
| `Control` | `SweetEditorControl` | UI framework convention uses `Control` | WinForms, WPF |
| `Widget` | `SweetEditorWidget` | UI framework convention uses `Widget` | Flutter, Qt |
| `View` | `SweetEditorView` | UI framework convention uses `View` | SwiftUI, UIKit, AppKit, React Native |
| `Component` | `SweetEditorComponent` | UI framework convention uses `Component` | Web Components, React, Vue |
| `Element` | `SweetEditorElement` | UI framework convention uses `Element` | Lit, Angular |

> Names without the `SweetEditor` prefix (e.g. `EditorControl`, `EditorView`, `EditorWidget`) are NOT allowed.
> If the host UI toolkit's conventional suffix is not listed above, a PR SHOULD be submitted to extend this table before implementation.
...
| `TextMeasurer` | OC: `SETextMeasurer` | May appear in any host-idiomatic form, including a top-level public type, nested type, internal bridge type, typealias, adapter, or C# struct, as long as the concept remains semantically aligned with the standard |
...
| `DecorationReceiver` | C#/TS/Kotlin: `IDecorationReceiver`; OC: `SEDecorationReceiver` | Callback interface; only applicable when the implementation exposes an explicit Receiver type |
| `CompletionReceiver` | C#/TS/Kotlin: `ICompletionReceiver`; OC: `SECompletionReceiver` | Callback interface; only applicable when the implementation exposes an explicit Receiver type |
...
| `EditorMetadata` | C#/TS/Kotlin: `IEditorMetadata`; OC: `SEEditorMetadata` | Metadata concept type; only applicable when the implementation exposes an explicit public type |
| `EditorEventListener` | C#/TS/Kotlin: `IEditorEventListener`; OC: `SEEditorEventListener` | Listener interface; applies only when the implementation exposes an explicit listener-interface pattern |
| `InlineSuggestionListener` | C#/TS/Kotlin: `IInlineSuggestionListener`; OC: `SEInlineSuggestionListener` | Listener interface; only applicable when the implementation exposes an explicit inline-suggestion listener |
...
| `SelectionMenuListener` | C#/TS/Kotlin: `ISelectionMenuListener`; OC: `SESelectionMenuListener` | Listener interface; only applicable when the implementation exposes an explicit selection-menu listener |
...
> If the event system uses host-native `event` / delegate / stream / signal APIs, the implementation does not need to expose public `EditorEventBus` / `EditorEventListener` types; in that case only the semantics in Section 11 are required.

### 2.2 Field / Property Names (MUST)

Data model fields MUST use the same semantic names across integrations, adapted to each language's casing convention:
...
For host-facing public APIs (such as `SweetEditor`, `SweetEditorController`, `EditorSettings`, event payloads, and provider / context / result types consumed directly by host code), implementations MUST use enums or equivalent strong types for discrete value sets when the target language supports them.

- Host-facing public APIs MUST NOT prefer raw `int` values when the language already supports enums / strong typed constants
- If language or framework constraints force a host-facing public API to expose integer constants, that layer MUST handle invalid values explicitly (see Section 15)
...
For simple geometry carriers used in public APIs and event payloads, implementations MAY use either the canonical SweetEditor geometry names or host-native equivalents when the semantics are identical.

- Point types: `PointF` or a host-native point type (e.g. Android `android.graphics.PointF`, Apple `CGPoint`)
- Rect types: `Rect` or a host-native rect type (e.g. Android `android.graphics.RectF`, Apple `CGRect`)

If a host-native geometry type is used, coordinate basis, axis direction, and field semantics MUST remain identical to the canonical SweetEditor model.

### 2.6 Range Effect Rendering Semantics (MUST)

`RangeEffectRenderItem` is the integration-rendered channel for visible range presentations such as selection, search, document highlights, IME composition, diagnostics, linked editing, and bracket matches.

- Implementations MUST treat `RangeEffectRenderItem.rect` as final screen-space geometry. Core has already split effects for wrapping, viewport visibility, folded-tail projection, and layout-specific cases.
- Implementations MUST NOT recompute range effect geometry from document text ranges.
- Implementations MUST draw `RangeEffectStyle.backgroundColor` before text runs.
- Implementations MUST draw `RangeEffectStyle.borderColor` and `RangeEffectStyle.underlineColor` after text runs and guide segments, and before the cursor.
- Implementations MUST NOT draw `RangeEffectStyle.foregroundColor` from `RangeEffectRenderItem`. Foreground effects are applied by core through `VisualRun.style` after run splitting.
- Implementations MAY use `RangeEffectKind` for small visual differences, such as a thicker active linked-editing border. Implementations MUST NOT infer editor state, event behavior, or provider behavior from `RangeEffectKind`.
- Implementations SHOULD keep underline presentation close to core renderer conventions: underline baseline near `rect.bottom - 1`, solid and dashed strokes around 2 px, wavy strokes around 3 px, dashed pattern around 3 px on / 2 px off, and wavy half-period / amplitude around 7 px / 3.5 px.
...
Implementations MUST implement every listed method on the appropriate API carrier unless a later section assigns a weaker requirement level to that method. Section 3.1 methods belong to `EditorCore`; they are not implicitly part of the host-facing editor surface. In imperative frameworks the Section 3.2 carrier is the widget entry class itself (for example `SweetEditor`), while in declarative frameworks the Section 3.2 carrier is `SweetEditorController`. On declarative integrations, `SweetEditor` remains the runtime/session owner even though the host-facing API is exposed through the controller.

> Lifecycle / memory management APIs (e.g. `create`, `destroy`, integration wrapper names for `free_binary_data`) are not listed here; each implementation implements them per its own conventions. Native binary payloads currently use the C ABI shape `const uint8_t* + out_size`; every implementation wrapper MUST release owned native result buffers after decoding.
...
|---|---|---|
| Imperative frameworks | **MUST** | The APIs in Section 3.2, plus host APIs from implemented optional modules, MUST be exposed directly on the widget entry class (for example `SweetEditor`, `SweetEditorView`, `SweetEditorControl`). Implementations MAY additionally provide `SweetEditorController` |
...
| Declarative initialization inputs | **MAY / MUST** | Declarative implementations MAY expose `document`, `text`, `theme`, `settings`, `keyMap`, and similar initialization inputs. If exposed, they MUST be construction/configuration inputs rather than pre-ready controller calls. `document` wins over `text`; text-only initialization MUST materialize an equivalent `Document`, and `getDocument()` MUST return it after ready |
| Explicit teardown | **MAY / MUST** | Implementations MAY provide terminal controller teardown such as `dispose()`, `close()`, or `release()`. If provided, it MUST release only controller-owned readiness callbacks, pending callbacks, and reference chains; it MUST NOT assume ownership of session provider registrations or runtime objects. After teardown, later calls MUST be no-ops or return default empty values |

---

### 3.1 `EditorCore` Public API

Section 3.1 defines the bridge/runtime API carried by `EditorCore`. It includes low-level render snapshot, gesture loop, keyboard dispatch, and animation tick methods. These methods are not part of the default host-facing editor surface unless an implementation explicitly chooses to expose `EditorCore` itself.

All state-changing `EditorCore` APIs, including configuration writes, gestures, keyboard input, text edits, IME writes, cursor/selection writes, scrolling/navigation, decoration, folding, linked editing, and animation ticks, MUST return `EditorActionResult` or a host-language equivalent. Query APIs keep their own semantic return values; `buildRenderModel()` is a render snapshot query and does not return `EditorActionResult`. Integration layers MUST pass every non-null `EditorActionResult` to one unified result dispatcher, and MUST NOT infer whether to emit text events, synchronize IME state, run animation, flush, or repaint from the invoked method name, setter category, or local side-effect assumptions.

During construction or first-frame bootstrap before the editor runtime / dispatcher exists, implementations MAY coalesce setup `EditorActionResult` values and perform equivalent state dispatch, IME synchronization, and refresh after the dispatcher is ready. This exception only applies to the initialization window where no dispatcher exists; every non-null `EditorActionResult` produced after runtime readiness MUST be processed through the unified dispatcher.
...
`EditorCore` IME APIs are bridge/runtime APIs. They standardize host input adaptation and testability; they are not required to be exposed on the host-facing `SweetEditor` / controller API, and an implementation is not required to call the complete function set. Conditional MUST means an implementation is not required to synthesize a native IME capability it does not receive, but if that capability exists it MUST map to the corresponding core semantic capability family.
...
`EditorOptions` is a bridge-layer configuration payload. Implementations MAY expose it as a public type or keep it internal, but if it crosses the bridge boundary or is serialized into a binary payload, the following field semantics and ordering MUST remain aligned with Core:
...
| `flingFriction` | float | implementation-defined | Fling friction coefficient; implementations MAY tune this to match native interaction feel |
| `flingMinVelocity` | float | implementation-defined | Minimum fling velocity in px/s; implementations MAY tune this to match native interaction feel |
| `flingMaxVelocity` | float | implementation-defined | Maximum fling velocity in px/s; implementations MAY tune this to match native interaction feel |
...
> If an implementation serializes `EditorOptions` into a binary bridge payload, field order MUST stay aligned with Core: `touch_slop`, `double_tap_timeout`, `long_press_ms`, `fling_friction`, `fling_min_velocity`, `fling_max_velocity`, `max_undo_stack_size`, `key_chord_timeout_ms`, `reveal_selection_end_on_select_all`.

### 3.2 Host-Facing Editor Public API

Section 3.2 defines the host-facing editor API. It intentionally excludes low-level `EditorCore` gesture-loop, animation-tick, and render-model production methods. Host-facing state-changing APIs may return `void`, `EditorActionResult`, or an integration-equivalent result according to integration conventions; whenever the underlying call produces a non-null `EditorActionResult`, the integration runtime MUST process it through the unified result dispatcher. `flush()` is no longer a required standard host-facing API. Implementations MAY keep it as a force-refresh, diagnostic, or compatibility API, but normal refresh/redraw decisions MUST come from `EditorActionResult.needsRedraw`. API carriers follow Section 3.0.

Except for items marked SHOULD / MAY, every host API listed below is MUST. Languages MAY use properties, delegate setters, typed streams, or host-idiomatic equivalent entrypoints when the mapping remains clear and unambiguous.
...
| Completion | `triggerCompletion()`, `showCompletionItems(items)`, `dismissCompletion()`, `setCompletionItemRenderer(renderer)` or an integration-equivalent completion item rendering customization API |
...
> The canonical naming for provider management methods is `add` / `remove`. Each implementation MAY use semantically equivalent variants per its own conventions (e.g. `attach` / `detach`, `register` / `unregister`).

> Clipboard methods (`copyToClipboard`, `pasteFromClipboard`, `cutToClipboard`) are **MAY** because clipboard access is target-specific.

> Event exposure does not require a uniform method shape. Implementations MUST provide a type-safe event mechanism per Section 11, and may use `subscribe` / `unsubscribe`, host-native `event` / delegates, typed `Stream` getters, signals / observers, or equivalent forms.
...
`CompletionItem` is the core data type of the completion system. Application priority on commit: `textEdit` → `insertText` → `label`. `textEdit` is the only implicit replacement range source; when it is absent, implementations MUST insert `insertText` / `label` at the current cursor rather than deriving a replacement range from `wordRange`. When `additionalTextEdits` is present without `textEdit`, implementations MUST use a collapsed cursor edit as `edits[0]`, then append `additionalTextEdits`.
...
| `newText` / `new_text` | String | **MUST** | Replacement text; implementations MAY expose idiomatic field casing while keeping the generated protocol field order unchanged |
...
| `setInlineSuggestionListener(listener)` | **MUST** | Register the host-visible accepted / dismissed listener; passing `null` clears it. Implementations MAY use callbacks, delegates, event subscriptions, or typed streams for the same semantics |

For the same shown suggestion instance, `accepted` and `dismissed` MUST each fire at most once. After either event fires, no further callbacks may be emitted for that instance. If `showInlineSuggestion()` replaces the current suggestion, the implementation MAY emit `dismissed` for the previous suggestion or replace it quietly, but the previous suggestion MUST NOT emit callbacks after replacement. After terminal editor teardown, internal detach, or controller disposal, no host-visible inline-suggestion callbacks may be emitted.
...
On mobile targets, the selection menu module is SHOULD level. Desktop implementations MAY omit it entirely. If implemented, it MUST follow the contract below.
...
All implementations MUST define `EditorTheme` with the following color fields. Field names follow Section 2.2 casing rules.
...
All color fields use the host color type (ARGB). Implementations MUST provide the following fields:
...
Every implementation MUST provide at least `dark()` and `light()` factory methods that return pre-configured themes. Built-in themes MUST assign explicit values for all required color fields, including selection text, search, document highlight, CodeLens, and link colors.
...
| `lineSpacingMult` | float | 1.2 | *(same as above)* | `getLineSpacingMult()` | `relayout` | Line spacing multiplier |
| `contentStartPadding` | float | Target-dependent | `setContentStartPadding(padding)` | `getContentStartPadding()` | `relayout` | Extra horizontal padding between gutter split and text rendering start (pixels) |
| `showSplitLine` | boolean | true | `setShowSplitLine(show)` | `isShowSplitLine()` | `repaint` | Whether to render the gutter split line |
| `gutterSticky` | boolean | Target-dependent | `setGutterSticky(sticky)` | `isGutterSticky()` | `repaint` | Whether gutter stays fixed during horizontal scroll (true=fixed, false=scrolls with content) |
...
>
> `readOnly` is a `runtime-transition` setting: when switching to read-only while an IME composition is active, the implementation MUST finish or cancel the active native composition before blocking subsequent edit requests.
>
> `contentStartPadding` is implementation-dependent by default. It MUST be `>= 0`. `0` is the neutral baseline, but implementations MAY choose a non-zero visual default.
>
> `gutterSticky` is implementation-dependent by default. Desktop-style implementations SHOULD default to `true`; mobile / touch-first implementations SHOULD default to `false`.
>
> `autoIndentMode`, `backspaceUnindent`, and `readOnly` are also `runtime-transition` settings. They MUST affect subsequent editing behavior immediately; if there is no visible state change at the moment of the setter call, the core-returned `EditorActionResult` should not ask the integration layer to refresh visible state.
...
All implementations MUST provide the core-layer keymap protocol types `KeyBinding`, `KeyChord`, `KeyCode`, `KeyModifier`, and `EditorBuiltinCommand`. Integration core APIs SHOULD accept the binding list directly and are not required to expose a separate core-layer `KeyMap` class. The C++ core may keep an internal `KeyMap` resolver; that internal type is not an integration public API contract.
...
If the implementation exposes `KeyCode`, `KeyModifier`, or `EditorBuiltinCommand` constants, their numeric values MUST align with the C++ core.
...
- For such bridge-layer integer enums, integrations are not required to repeat host-facing business-level enum validation, but MUST ensure invalid input cannot cause native / C++ crashes or undefined behavior

### 10.3 Widget-Layer Extension

- `SweetEditor` MUST support `setKeyMap(keyMap)` and SHOULD expose `getKeyMap()`
- Implementations MUST expose `EditorKeyMap` as a widget-layer shortcut container so host code can bind command ids to host-side handlers
...
- Implementations MAY additionally expose convenience APIs for custom-command registration, but `registerCommand(binding, handler)` remains the canonical contract
- Auto-assigned custom command ids MUST be greater than `BUILT_IN_MAX`
- Implementations MUST provide `defaultKeyMap()` as the default binding factory
- Implementations MUST provide `vscode()`, and `defaultKeyMap()` MUST be semantically equivalent to `vscode()`
- Implementations SHOULD provide named preset factories such as `jetbrains()` and `sublime()`
- `SweetEditor.setKeyMap()` MUST replace the current keymap and make the new bindings immediately effective
- Widget-layer handlers remain integration-side and are not serialized to the C++ core
- If an implementation does not expose `getKeyMap()`, its documentation MUST clearly describe how host code constructs and replaces the active keymap
...
All implementations MUST provide a **type-safe** editor event exposure mechanism so host code can observe specific event types and manage subscription lifecycle through unsubscribe / dispose / cancel-listening or an equivalent operation.

Implementations MAY use any of the following forms:
- `EditorEventBus` + `subscribe` / `unsubscribe` / `clear`
- Host-native event / delegate / listener mechanisms (for example C# `event`, Java listener callbacks)
- Typed stream / signal / observable getters (for example Dart `Stream<T>`)

If an implementation adopts an explicit event-bus/listener pattern, the related public types SHOULD be named `EditorEventBus` / `EditorEventListener`.

The host-visible event surface is an observer surface. Implementations MAY keep `publish` / `emit` or equivalent methods internally, but state events such as text, cursor, selection, scroll, and scale MUST be published by the unified `EditorActionResult` dispatcher. Host-facing APIs SHOULD NOT expose an entry point that can publish arbitrary editor state events.

### 11.2 Required Event Types

All implementations MUST support the following event types:
...
LongPressEvent,       // mobile / touch targets, including iOS, Android, and OHOS
DoubleTapEvent,
ContextMenuEvent      // targets with an explicit context-menu gesture entry point
```

> `LongPressEvent` is for mobile / touch targets, including iOS, Android, and OHOS, and represents the raw long-press gesture itself. `ContextMenuEvent` is for targets that expose an explicit context-menu gesture entry point (for example desktop right click or a framework-native context-menu gesture). Integration implementations SHOULD only register events relevant to their target.

> The above event types MUST be distinguishable and consumable in a type-safe way through the integration's chosen event mechanism.

Target-specific events (e.g. `SelectionMenuItemClickEvent` on mobile) MAY be added.

`DocumentLoadedEvent` is a document lifecycle event and does not have to be derived directly from an `EditorActionResult` field. Implementations MAY publish it from the `loadDocument(...)` lifecycle path. However, any non-null `EditorActionResult` returned by `loadDocument(...)` MUST still be passed to the unified dispatcher; `DocumentLoadedEvent` must not replace result dispatch.

### 11.3 Event Payload Contract

Event payloads MUST be defined per-event. Implementations MUST NOT assume or require a shared base payload schema beyond the event type itself.
...
| `FoldToggleEvent` | `line: int`, `isGutter: boolean`, `locationInEditor: PointF or host-native point type` | Toggled fold line, whether the click came from gutter, and pointer location relative to the editor's local coordinate space |
| `GutterIconClickEvent` | `line: int`, `iconId: int`, `locationInEditor: PointF or host-native point type` | Clicked gutter icon line, icon id, and pointer location relative to the editor's local coordinate space |
| `InlayHintClickEvent` | `line: int`, `column: int`, `type: InlayType`, `intValue: int`, `locationInEditor: PointF or host-native point type` | Clicked inlay hint position, inlay type, type-specific value, and pointer location relative to the editor's local coordinate space |
| `CodeLensClickEvent` | `line: int`, `column: int`, `commandId: int`, `locationInEditor: PointF or host-native point type` | Clicked CodeLens line/column anchor, unique command id, and pointer location relative to the editor's local coordinate space |
| `LinkClickEvent` | `line: int`, `column: int`, `target: String`, `locationInEditor: PointF or host-native point type` | Clicked link line/column anchor, resolved link target, and pointer location relative to the editor's local coordinate space |
| `LongPressEvent` | `cursorPosition: TextPosition`, `locationInEditor: PointF or host-native point type` | Raw long-press target position and pointer location relative to the editor's local coordinate space |
| `DoubleTapEvent` | `cursorPosition: TextPosition`, `hasSelection: boolean`, `selection: TextRange?`, `locationInEditor: PointF or host-native point type` | Double-tap target position, resulting selection state, and pointer location relative to the editor's local coordinate space |
| `ContextMenuEvent` | `cursorPosition: TextPosition`, `locationInEditor: PointF or host-native point type` | Explicit context-menu gesture target position and pointer location relative to the editor's local coordinate space |
| `ContextMenuItemClickEvent` *(target-specific)* | `item: ContextMenuItem`, `request: ContextMenuRequest` | Clicked custom context-menu item and the immutable request snapshot used to build that menu |
| `SelectionMenuItemClickEvent` *(target-specific)* | `item: SelectionMenuItem` | Clicked custom selection-menu item |

### 11.4 `EditorActionResult` Gesture Field Contract

Implementations MAY expose the return value of `handleGestureEvent(...)` directly, or consume it internally only; however, the gesture return value MUST be `EditorActionResult` or an integration-equivalent type. The following gesture-related fields MUST keep semantics consistent with Core and be consumed by the unified result dispatcher:
...
`ContextMenu` is a widget-layer, integration-side UI capability. It MUST NOT be modeled as a C++ Core render-model concept or serialized as a Core decoration type. If an implementation implements context menus, it MUST follow the following standard data model and semantics.
...
| `locationInEditor` | `PointF or host-native point type` | **MUST** | Pointer location relative to the editor's local coordinate space |
...
| `hitTarget` | integration-aligned `HitTarget` payload | **MUST** | Hit-test result at the trigger location |
...
| `icon` | host-native leading icon object or equivalent | **MAY** | Optional leading icon. This SHOULD be a host-native image object, not a cross-platform numeric icon id |
...
- Passing `null` as the provider SHOULD restore the host default context menu
- Returning an empty list MAY suppress the menu for that show cycle
- `locationInEditor` MUST remain editor-local; implementations convert it to screen / window coordinates only when presenting a popup or native menu
- An implementation MAY open a context menu from `LongPressEvent` without publishing `ContextMenuEvent`; in that case `ContextMenuRequest.triggerKind` MUST still be `LONG_PRESS`
...
- `ContextMenuItem.icon` MAY be null; if a given menu contains any icon-bearing rows, implementations SHOULD reserve a consistent leading icon slot for visual alignment
- The implementation MUST provide a host-visible way to observe custom item activation via `ContextMenuItemClickEvent` or an equivalent callback payload
- After command execution, text change, or another gesture that invalidates the current target, the menu SHOULD dismiss unless the implementation intentionally supports a persistent multi-step workflow

Implementations that provide `ContextMenu` MUST expose a host-facing API equivalent to `setContextMenuItemProvider(provider)`.

---

## 12. Enumeration and Constant Values (MUST)

Enum and enum-like constant values MUST match the C++ core definitions. The following groups MUST remain aligned with the C++ core across all implementations. When explicit numeric values are listed below, implementations MUST use those same values. When a row lists only member names or says it is aligned with the C++ core, implementations MUST still match the corresponding core definition.
...
| `GuideDirection` | (integration-aligned with C++ core) |
...
## 13. Implementation-Specific Allowances

### 13.1 Bridge Layer (MAY differ)

Each implementation uses its own native bridge technology. This is expected and not constrained:

| Implementation | Bridge Technology |
|---|---|
...
- Host-native font rendering differences

---

## 14. Threading and Concurrency Model (MUST)

State-mutating editor operations and host-visible callbacks are UI-thread-affine by default. Implementations MAY expose additional thread-safe query surfaces, but MUST choose a concrete threading model; implementations SHOULD explain that model through code comments, type annotations, or a README.

| Rule | Constraint | Description |
|---|---|---|
| State-mutating API thread | **MUST** | Public methods that mutate editor state or trigger visible UI updates MUST be called on the UI thread unless the implementation explicitly documents an equivalent serialized threading model |
| API thread contract documentation | **SHOULD** | Implementations SHOULD use code comments, type annotations, or a README to identify which public APIs are UI-thread-only and which pure query / snapshot APIs, if any, are safe to call from background threads |
| Pure query API thread | **SHOULD** | Pure query / snapshot APIs SHOULD either remain UI-thread-only or be explicitly documented as background-safe; implementations MAY allow background reads only when implemented safely |
...
| Provider call thread | **MUST** | Implementations MUST choose a stable invocation model for `provideDecorations()` and `provideCompletions()` (UI thread, worker thread, or another serialized executor); implementations SHOULD explain that model to host code through code comments, type annotations, or a README |
...
| `buildRenderModel()` | **MUST** | `buildRenderModel()` MUST observe a stable editor snapshot. Implementations MAY require UI-thread calls or provide a stronger thread-safe snapshot contract, but the returned `EditorRenderModel` SHOULD be treated as immutable and MAY be safely read on the render thread |
...
| Thread safety annotations | **SHOULD** | Implementations SHOULD annotate thread constraints in public API documentation (e.g. Java `@MainThread`, Swift `@MainActor`) |
...
| null / empty parameters | **MUST** | Implementations MUST honor the nullable semantics of parameters that are defined as nullable. For MUST-non-null parameters, managed-language public APIs SHOULD fail fast using host-idiomatic errors (for example Java `NullPointerException` / `IllegalArgumentException`, C# `ArgumentNullException`) and MUST NOT cause native / C++ crashes or undefined behavior; bridge / FFI boundaries MUST handle invalid input safely |
| Invalid enum values | **MUST** | For host-facing public APIs that are forced to expose integer enum values, implementations MUST handle invalid values explicitly; managed-language public APIs SHOULD fail fast using host-idiomatic errors (for example `IllegalArgumentException`), and MAY instead fall back to a default value. For raw integer enum values used by `EditorCore`, bridge layers, or FFI layers, integrations are not required to repeat host-level business validation, but MUST NOT allow invalid input to cause native / C++ crashes or undefined behavior |
| Calls outside ready / active lifecycle | **SHOULD** | Implementations SHOULD follow the lifecycle rules in Sections 3.0.3 and 16.3. Before a declarative editor instance is ready, or after terminal teardown, getters SHOULD return `null` or default values. Mutating imperative calls MUST be ignored or rejected and MUST NOT be queued. After terminal teardown, runtime-affecting calls MUST be no-ops or return default values. This rule primarily applies to declarative controllers, explicit teardown APIs, or implementations with a defined terminal session lifecycle boundary |

### 15.2 Provider Exception Handling

| Rule | Constraint | Description |
|---|---|---|
| Exception capture | **SHOULD** | Implementations SHOULD isolate Provider exceptions where practical so a single Provider does not affect other Providers or crash the editor; for Providers on the synchronous input hot path (such as `NewLineActionProvider`), implementations MAY skip a uniform try-catch wrapper and use a lighter or host-native strategy instead |
| Exception logging | **MAY** | Implementations MAY log caught exceptions; the standard does not require any specific log format or fields such as Provider class name |
...
| Rule | Constraint | Description |
|---|---|---|
| Bridge layer error conversion | **MUST** | Error codes returned by C++ Core MUST be converted to host-idiomatic error representations in the bridge layer (e.g. Java logging + no-op, Swift `Result` type); MUST NOT propagate C++ exceptions directly to upper layers |
...
For all implementations, the conformance target is terminal release-path safety plus eventual native-resource release. The standard does not require every implementation to expose an explicit `dispose()` / `close()` / `release()` API, and it does not require a single cross-platform deterministic destruction moment. Implementations MAY satisfy the release-path requirement through an explicit teardown API, a host-managed lifecycle, widget or controller destruction, destructor / RAII / `Drop`, ARC / `deinit`, GC / finalizer-backed reclamation, or another host-idiomatic cleanup mechanism. For GC-managed imperative widget integrations, the standard does not require inventing a synthetic terminal session-teardown hook solely for conformance; the primary requirement is eventual native-resource release plus safety after teardown or release.
...
| Release path | **MUST** | The implementation MUST ensure that `EditorCore` and its native / C++ resources are eventually released. An explicit `dispose()` / `close()` / `release()` API is optional. Implementations MAY instead rely on a host-managed editor lifecycle, widget/session destruction, destructor / RAII / `Drop`, ARC / `deinit`, GC / finalizer-backed automatic reclamation, an equivalent host cleanup hook, or another host-idiomatic strategy. For GC-managed imperative widget integrations, eventual reclamation through GC, finalizer, Cleaner, or an equivalent runtime-backed cleanup mechanism is sufficient even when the implementation does not expose a distinct terminal session callback. Controller destruction only counts when it is part of the associated editor instance's terminal cleanup path. View detachment, widget unmount, or temporary removal from the view tree is NOT by itself required to be the final reclamation moment |
| Post-teardown calls | **MUST** | If the implementation exposes an explicit terminal teardown API, or otherwise keeps the object reachable and callable after logical teardown or internal release, subsequent calls MUST NOT access invalid native / C++ resources and MUST NOT trigger further editor side effects or callbacks. Mutating calls MUST be no-ops or return default values. Getter calls MAY return `null`, default values, or last-known managed snapshots, as long as they do not require live native state or trigger lazy recomputation against released resources |
| Repeated release | **MUST** | If the implementation exposes explicit release logic, multiple invocations MUST be idempotent (no-op); MUST NOT cause double-free |

> The standard requires eventual native-resource release, but does **not** require every implementation, or every `Document` / bridge wrapper, to expose an additional explicit release API beyond its own lifecycle model. Implementations SHOULD prioritize logical teardown safety: stop timers, detach listeners, cancel or stale-mark async receivers, and break reference chains that would otherwise keep the editor object graph alive. If an implementation exposes explicit teardown logic or another known terminal cleanup callback, it MUST perform the corresponding logical teardown cleanup. On GC-managed imperative widget integrations that do not expose such a terminal cleanup hook, proactive cleanup remains a SHOULD rather than a MUST. If an implementation chooses to keep returning last-known managed snapshots after teardown, it SHOULD document that those values are stale snapshots rather than live editor state.
...
| Cleanup during terminal teardown | **MUST** | If the implementation defines an explicit controller `dispose()` / `close()` / `release()` phase, or another equivalent final logical teardown hook, it MUST clear controller-owned readiness callbacks and internal pending callbacks, and cancel or stale-mark any controller-owned async work so late results are ignored. It MUST NOT imply controller-side ownership of session provider registrations. On GC-managed integrations, when such an explicit or host-native terminal cleanup hook exists, the equivalent logical teardown MUST still detach listeners, stop timers, and cancel or stale-mark async receivers so late results cannot keep the editor object graph alive or mutate freed native resources / host-visible editor state. If no such terminal cleanup hook exists on a GC-managed imperative integration, proactive cleanup remains SHOULD rather than MUST |
| Provider references | **SHOULD** | Integration implementations SHOULD avoid Providers holding strong references to the widget instance to prevent circular references causing memory leaks (Java/Kotlin: WeakReference; Swift: weak/unowned; Dart: no special handling needed) |
...
| Explicit teardown (if provided) | **MAY** | Implementations MAY provide an explicit terminal controller teardown method such as `dispose()`, `close()`, or `release()`. This is optional for both GC-managed and non-GC integrations when terminal teardown is already guaranteed by the host lifecycle or by host-native destruction semantics |
...
- `EditorCore` / native resources MUST be released exactly once and only after no further integration callbacks can legally use them
- Target-specific resources (textures, canvases, timers, etc.) MAY be released in host-idiomatic order, as long as the constraints above are preserved
...
The Core layer defines numerous decoration data types. All implementations MUST implement identical fields. This section specifies the MUST fields, construction constraints, and immutability requirements for each data type.
...
| `GutterIcon` | `iconId` | Icon resources are resolved and rendered by the integration's `EditorIconProvider` |
...
| Type | MUST fields | Special semantics |
|---|---|---|
| `EditorRenderModel` | `pointerCursorType` | MUST on desktop or targets with mouse / hover input, MAY on touch-only targets; should stay semantically consistent with `EditorActionResult.pointerCursorAfter` |
...
All implementations MUST support at least the following two construction methods:

| Method | Constraint | Description |
|---|---|---|
| From string | **MUST** | `Document(text: String)` - create from in-memory text content |
| From file path | **SHOULD** | `Document(file: File)` / `Document(path: String)` - create from a local file; large-file loading strategy is target-specific |

> Constructor parameter naming and types MAY vary by implementation (e.g. Java `File`, C# `string path`, Swift `URL`), but semantics MUST be consistent.
...
| Resource release | **MUST** | When `Document` reaches its terminal host lifecycle state, the bridge layer MUST eventually release the C++ side document memory. The exact cleanup mechanism is target-specific, and an explicit `dispose()` / `close()` API is optional on both GC-managed and non-GC integrations |
| Encoding model | **MUST** | Integration layers MUST NOT assume or expose a specific internal storage / layout encoding beyond the semantics guaranteed by the public APIs |
...
`EditorMetadata` is a **semantic concept type** representing host-defined metadata attached to an editor instance. The integration layer is responsible only for storing and returning it, not interpreting its internal structure.

| Rule | Constraint | Description |
|---|---|---|
| Representation form | **MUST** | Implementations MUST provide some representation capable of carrying arbitrary host-defined metadata; they MAY use a marker interface / protocol / abstract class / base class / `Object` / `any` / `unknown` / generic payload, etc. |
| Explicit type naming | **SHOULD** | If the implementation chooses to expose an explicit public type, it SHOULD be named `EditorMetadata`; language-conventional variants such as `IEditorMetadata` or `SEEditorMetadata` are also allowed |
| Purpose | **MUST** | Host code stores and retrieves custom metadata (e.g. file path, language ID) via `setMetadata()` / `getMetadata()` or language-idiomatic typed variants such as `setMetadata<T>()` / `getMetadata<T>()`; the integration layer MUST treat it as an opaque value and MUST NOT impose its own schema |
| Retrieval semantics | **MUST** | `getMetadata()` or its typed variant MUST return the same metadata value previously set when the requested type matches, or `null` if none exists; if the implementation exposes a wider carrier type (such as `Object?`), host code is responsible for its own casts / type assertions |
...
| `brackets` | List\<BracketPair\>? | **MAY** | Bracket pair list (null = not configured; implementation MUST NOT sync to Core when null) |
| `autoClosingPairs` | List\<BracketPair\>? | **MAY** | Auto-closing bracket pair list (null = not configured; implementation MUST NOT sync to Core when null) |
...
| Optionality and defaults | **MUST** | Implementations MAY expose `tabSize` / `insertSpaces` as nullable or non-null fields. If nullable, `null` MAY mean "use editor default". If non-null, their default values MUST match the editor defaults |
...
This section defines the performance invariants that integration implementations must preserve. Numeric targets are optimization goals, not conformance gates.

| Rule | Constraint Level | Description |
|---|---|---|
| Viewport-scoped rendering | **MUST** | Layout and painting on the integration side MUST be scoped to the visible region (plus small lookahead buffers when needed); implementations MUST NOT require full-document relayout or redraw for ordinary scrolling |
...
| Core/layout duplication | **MUST** | Integration hot paths MUST NOT redundantly recompute geometry or layout information that is already produced by Core and can be consumed directly |
| Performance diagnostics | **SHOULD** | Implementations SHOULD preserve enough timing hooks to support debug-only performance diagnostics |
...
| Integration API tests | **SHOULD** | Each implementation SHOULD verify host APIs, settings/theme defaults, provider registration/release, and event subscription/release |
| IME regression matrix | **MUST / SHOULD** | IME protocol semantics MUST be covered by core regression tests, and each implementation SHOULD validate native IME behavior with the [`ime-regression-matrix.md`](ime-regression-matrix.md) checklist before releasing IME changes |
| Result dispatch tests | **SHOULD** | Each implementation SHOULD cover `EditorActionResult` dispatch, IME synchronization, text / cursor / selection / scroll events, animation ticks, and pointer cursor updates |
| Async stale tests | **SHOULD** | Stale decoration / completion async results SHOULD be cancelled or discarded |
| Lifecycle tests | **SHOULD** | After teardown, integrations should not touch invalid native state or emit host-visible callbacks |
...
| Rule | Constraint Level | Description |
|---|---|---|
| Role annotation | **SHOULD** | The editor widget SHOULD be annotated as a text editor or integration-equivalent role |
...
| Keyboard shortcuts | **SHOULD** | Desktop implementations SHOULD support standard keyboard shortcuts (Ctrl/Cmd+C/V/X/Z/A, etc.) |
...
This standard applies to all SweetEditor integration implementations. When the C++ core adds new enums, events, or API methods, all implementations MUST be updated to match within the same release cycle.

### 23.1 Integration Package Version Numbering

Integration package versions use Semantic Versioning: `a.b.c` with an optional prerelease suffix such as `1.0.0-rc1`.

| Segment | Constraint | Rule |
|---|---|---|
| `a` (major) | **MUST** | Packages in the same coordinated release MUST use the same major version |
| `b` (minor) | **MUST** | Packages in the same coordinated release MUST use the same minor version |
| `c` (patch) | **MUST** | Packages in the same coordinated release MUST use the same patch version |
| Prerelease | **MUST** | Coordinated prereleases MUST use the same identifier, such as `rc1` |

- A coordinated repository release uses one version across the core artifacts and all integration packages.
- An implementation MAY publish an independent patch only for a target-specific fix. The release notes MUST identify the core revision and explain why the version differs from the coordinated release.
