...
internal sealed class MainView : UserControl
{
    ...
    private const double ToolbarMenuPopupWidth = 168;
    ...
    private readonly Border toolbarMenuPopup = new();
    private readonly StackPanel toolbarMenuItemsPanel = new();
    ...
    private void BuildLayout()
    {
        ...
        toolbarContainer.Child = toolbarController.BuildView(IsMobilePlatform());
        BuildSearchPanel();
        BuildToolbarMenuPopup();
        ...
        toolbarMenuPopup.Child = toolbarMenuItemsPanel;
        toolbarMenuPopup.IsVisible = false;
        toolbarMenuPopup.Padding = new Thickness(6);
        toolbarMenuPopup.CornerRadius = new CornerRadius(8);
        toolbarMenuPopup.BorderThickness = new Thickness(1);
        toolbarMenuPopup.HorizontalAlignment = HorizontalAlignment.Left;
        toolbarMenuPopup.VerticalAlignment = VerticalAlignment.Top;
        toolbarMenuPopup.ZIndex = 80;
        ...
        Grid.SetRowSpan(toolbarMenuPopup, 4);
        ...
        layoutRoot.Children.Add(toolbarMenuPopup);
        Content = layoutRoot;
    }

    private void BuildToolbarMenuPopup()
    {
        toolbarMenuItemsPanel.Spacing = 2;
        toolbarMenuItemsPanel.Children.Clear();
        toolbarMenuItemsPanel.Children.Add(CreateToolbarMenuItem("Search", () => OpenSearchPanel(replaceMode: false)));
        toolbarMenuItemsPanel.Children.Add(CreateToolbarMenuItem("Replace", () => OpenSearchPanel(replaceMode: true)));
        toolbarMenuItemsPanel.Children.Add(CreateToolbarMenuItem("Wrap", CycleWrapMode));
        toolbarMenuItemsPanel.Children.Add(CreateToolbarMenuItem("Perf", TogglePerfOverlay));
        toolbarMenuItemsPanel.Children.Add(CreateToolbarMenuItem("Help", ShowKeyboardShortcutsDialog));
    }

    private Button CreateToolbarMenuItem(string text, Action action)
    {
        Button button = new()
        {
            Content = text,
    ...
    private void WireEvents()
    {
        ...
        toolbarController.MoreButton.Click += (_, _) => ToggleToolbarMenuPopup();
    ...
    private void ApplyTheme()
    {
        ...
        toolbarMenuPopup.Background = CreateBrush(darkTheme ? 0xFF1B1E24u : 0xFFFFFFFFu);
        toolbarMenuPopup.BorderBrush = CreateBrush(darkTheme ? 0xFF313844u : 0xFFD7DEE8u);
        foreach (Button button in toolbarMenuItemsPanel.Children.OfType<Button>())
        {
    ...
    private void ToggleToolbarMenuPopup()
    {
        if (toolbarMenuPopup.IsVisible)
        {
            HideToolbarMenuPopup();
            return;
        }

        toolbarMenuPopup.IsVisible = true;
        HideSamplePickerPopup();
        ScheduleChromeRefresh(DeferredChromeWork.ToolbarMenuPopup);
    }

    private void HideToolbarMenuPopup()
    {
        toolbarMenuPopup.IsVisible = false;
    }
    ...
    private void RepositionToolbarMenuPopup()
    {
        if (!toolbarMenuPopup.IsVisible)
            return;

        if (!TryGetControlBoundsInLayoutRoot(toolbarController.MoreButton, out AvaloniaRect buttonBounds))
            return;
        ...
        double width = Math.Min(ToolbarMenuPopupWidth, Math.Max(120, rootWidth - margin * 2));
        toolbarMenuPopup.Width = width;
        toolbarMenuPopup.Measure(new AvaloniaSize(width, double.PositiveInfinity));
        double height = toolbarMenuPopup.DesiredSize.Height;

        double x = Math.Clamp(buttonBounds.Right - width, margin, Math.Max(margin, rootWidth - width - margin));
        double y = buttonBounds.Bottom + height + margin > rootHeight
            ? Math.Max(margin, buttonBounds.Y - height - 4)
            : Math.Max(margin, buttonBounds.Bottom + 4);
        toolbarMenuPopup.Margin = new Thickness(x, y, 0, 0);
    }
    ...
    {
        if (!samplePickerPopup.IsVisible && !toolbarMenuPopup.IsVisible)
            return;

        Point point = e.GetPosition(layoutRoot);
        if (samplePickerPopup.IsVisible &&
            !IsPointInsideControl(toolbarController.SamplePickerButton, point) &&
            !IsPointInsideControl(samplePickerPopup, point))
        {
            HideSamplePickerPopup();
        }

        if (toolbarMenuPopup.IsVisible &&
            !IsPointInsideControl(toolbarController.MoreButton, point) &&
            !IsPointInsideControl(toolbarMenuPopup, point))
        {
            HideToolbarMenuPopup();
        }
    }

    private bool IsPointInsideControl(Control control, Point point)
    {
        return TryGetControlBoundsInLayoutRoot(control, out AvaloniaRect bounds) && bounds.Contains(point);
    }
    ...
    private void OnEditorKeyDown(object? sender, KeyEventArgs e)
    {
        ...
        if (toolbarMenuPopup.IsVisible && e.Key == Key.Escape)
        {
            HideToolbarMenuPopup();
            e.Handled = true;
            return;
    ...
    private void ApplyScheduledChromeRefresh()
    {
        ...
        if ((work & DeferredChromeWork.ToolbarMenuPopup) != 0 && toolbarMenuPopup.IsVisible)
            RepositionToolbarMenuPopup();
