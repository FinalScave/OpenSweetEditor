...
public struct SelectionMenuItem: Hashable {
...
public protocol SelectionMenuItemProvider: AnyObject {
    func provideSelectionMenuItems(for editor: SweetEditorView) -> [SelectionMenuItem]
}

final class SelectionMenuController {
    private final class MenuButton: UIButton {
        var menuItem: SelectionMenuItem?
    }
    ...
    init(editor: SweetEditorView, theme: EditorTheme) {
        self.editor = editor
        configureViews()
        applyTheme(theme)
    }
    ...
    private func rebuildItems(_ items: [SelectionMenuItem]) {
        for view in stackView.arrangedSubviews {
    ...
    private func onItemSelected(_ item: SelectionMenuItem) {
